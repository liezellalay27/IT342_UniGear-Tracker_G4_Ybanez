...
function MyRequests() {
  ...
  // Fetch equipment to derive categories for the request form
  useEffect(() => {
    const loadCategories = async () => {
      try {
        ...
        const data = await response.json();
        const unique = Array.from(new Set((data || []).map((e) => e.category))).sort((a, b) => a.localeCompare(b));
  ...
  if (loading) {
    ...
    return <LoadingSpinner message="Loading requests..." showTimer={false} />;
  }
          ...
          <div className={styles.requestForm}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className={styles.formGroup}>
                ...
                <p className={styles.formLabelHint}>Upload a PDF file for event approval (if applicable)</p>
                <input
                  type="file"
                  accept=".pdf"
                ...
                {formData.eventApprovalPdf && (
                  <div className={styles.fileSelected}>✓ {formData.eventApprovalPdf.name}</div>
                )}
              </div>
        ...
        <div className={styles.requestsGrid}>
          {displayedRequests.length === 0 ? (
            <div className={styles.noRequests}>
              ...
              {activeTab === 'active' && (
                <button onClick={() => setShowForm(true)} className={styles.btnPrimary}>Create Your First Request</button>
              )}
            </div>
          ) : (
            displayedRequests.map(request => {
              const getBorderClass = () => {
                switch(request.status) {
                  ...
                  case 'APPROVED': return styles.borderApproved;
                  case 'REJECTED': return styles.borderRejected;
                  case 'COMPLETED': return styles.borderCompleted;
                  default: return '';
                }
              };
              ...
              const getReturnStatusClass = () => {
                if (request.returnedOnTime === true) {
                  return `${styles.returnBadge} ${styles.onTime}`;
                }
                if (request.returnedOnTime === false) {
                  return `${styles.returnBadge} ${styles.late}`;
                }
                return `${styles.returnBadge} ${styles.unknown}`;
              };

              return (
              <div key={request.id} className={`${styles.requestCard} ${getBorderClass()}`}>
                <div className={styles.requestCardHeader}>
                  <h3 className={styles.requestCardTitle}>🎯 {request.equipmentName}</h3>
                ...
                {request.description && (
                  <div className={styles.requestSection}>
                    ...
                    <p className={styles.sectionContent}>{request.description}</p>
                  </div>
                )}
                ...
                <div className={styles.requestDetailsGrid}>
                  ...
                  <div className={styles.detailItem}>
                    ...
                    <span className={styles.detailValue}>{request.quantity}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>📥 Borrow Date</span>
                  ...
                  <div className={styles.detailItem}>
                    ...
                    <span className={styles.detailValue}>{request.studentName || '-'}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>🪪 School ID</span>
                    <span className={styles.detailValue}>{request.schoolIdNumber || '-'}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>🎓 Year</span>
                    <span className={styles.detailValue}>{request.yearLevel || '-'}</span>
                  </div>
                  <div className={styles.detailItem}>
                    <span className={styles.detailLabel}>📚 Course</span>
                    <span className={styles.detailValue}>{request.course || '-'}</span>
                  </div>
                  {request.status === 'COMPLETED' && (
                    <div className={styles.detailItem}>
                      <span className={styles.detailLabel}>⏱ Return Status</span>
                      <span className={getReturnStatusClass()}>
                ...
                {request.notes && (
                  <div className={styles.requestSection}>
                    <div className={styles.sectionLabel}>💬 Admin Notes:</div>
                    <p className={`${styles.sectionContent} ${styles.adminNote}`}>{request.notes}</p>
                  </div>
                )}

                {request.eventApprovalPdf && (
                  <div className={styles.requestSection}>
                    <div className={styles.sectionLabel}>📄 Event Approval PDF:</div>
                    <button 
